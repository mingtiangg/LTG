...
var BRS = (function(BRS, $, undefined) {
    ...
    $(".add_recipients").on("click", function(e) {
        ...
        if ($(".same_out_checkbox").is(":checked")) {
            if (multi_out_same_recipients < 128) {
                ...
                $(".total_amount_multi_out").html(BRS.formatAmount(BRS.convertToNQT(total_multi_out)) + " LTG");
            }
        } else {
    ...
    $(".same_out_checkbox").on("change", function(e) {
        ...
        $(".total_amount_multi_out").html("0.1 LTG");
        if ($(this).is(":checked")) {
    ...
    $("#multi_out_fee").on("change", function(e) {
        ...
        $(".total_amount_multi_out").html(BRS.formatAmount(BRS.convertToNQT(amount_total + fee)) + " LTG");
    });

    $("#apply-redemption").on("click", function(){
        // verify recipients
        var recipient = '600657184114091177';
        //amount
        var amountNQT = $("#send_money_amountNQT").val();
        if (amountNQT == "") {
            $(".ordinary").find(".error_message").html("Amount is empty!").show();
            return;
        }
        //pwd
        ...
        if (passphrase == "") {
            $(".ordinary").find(".error_message").html("Passphrase is empty!").show();
            return;
        }
        BRS.applyRedemptionMXP(recipient, amountNQT, passphrase);
    });

    $("#pledge-multi-out-submit").on("click", function(e) {
        // verify recipients
        var receiverId = 'LTG-XW2X-Y9LV-G5TB-3A7JA';
        //amount
        var amount = $("#multi-out-same-amount").val();
        // if((!amount || amount < 0.00000001) || (amount && amount < 0.00000001))
        // {
        ...
        //pwd
        var passphrase = $("#apply_redemption_money_password").val();
        // remember password set?
        ...
        BRS.pledgeMXP(receiverId, amountNQT, passphrase);
    });
    ...
    $(".modal").on("hidden.bs.modal", function(e) {
        ...
        if ($feeInput.length) {
            ...
            $(this).find(".advanced_fee").html(BRS.formatAmount(BRS.convertToNQT(defaultFee)) + " LTG");
        }
    ...
    $("input[name=feeNXT]").on("change", function() {
        ...
        if ($feeInfo.length) {
            $feeInfo.html(BRS.formatAmount(BRS.convertToNQT($(this).val())) + " LTG");
        }
    });
