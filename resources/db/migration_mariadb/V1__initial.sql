...
CREATE TABLE `transaction` (
  ...
  `trans_type` tinyint(1) default 0,
  `ransom_time` bigint(20) DEFAULT NULL,
  PRIMARY KEY (`db_id`),
...
DROP TABLE IF EXISTS `account_ransom`;
CREATE TABLE `account_ransom` (
  `db_id` bigint(20) NOT NULL AUTO_INCREMENT,
  `account_id` bigint(20) NOT NULL,
  `ransom`   bigint(20) default 0,
  `apply_time` bigint(20) default 0,
  `height` int(11),
  `latest` tinyint(1) NOT NULL DEFAULT '1',
  PRIMARY KEY (`db_id`),
  KEY `account_id_apply_idx` (`account_id`,`apply_time`),
  KEY `apply_ransom_status_idx` (`apply_time`,`ransom`,`latest`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
