...
public class SqlAccountStore implements AccountStore {
    ...
    private static final DbKey.LinkKeyFactory<Account.AccountRansom> accountRansomDbKeyFactory
            = new DbKey.LinkKeyFactory<Account.AccountRansom>("account_id", "apply_time") {
        @Override
        public DbKey newKey(Account.AccountRansom accountRansom) {
            return (DbKey) accountRansom.burstKey;
        }
    };
    ...
    private final VersionedEntityTable<Account.AccountRansom> accountRansomTable;
    ...
    public SqlAccountStore(DerivedTableManager derivedTableManager, DBCacheManagerImpl dbCacheManager) {
        ...
        accountRansomTable = new VersionedEntitySqlTable<Account.AccountRansom>("account_ransom", brs.schema.Tables.ACCOUNT_RANSOM, accountRansomDbKeyFactory, derivedTableManager) {
            private final List<SortField<?>> sort = initializeSort();

            private List<SortField<?>> initializeSort() {
                List<SortField<?>> sort = new ArrayList<>();
                sort.add(tableClass.field("account_id", Long.class).asc());
                sort.add(tableClass.field("apply_time", Long.class).desc());
                return Collections.unmodifiableList(sort);
            }

            @Override
            protected Account.AccountRansom load(DSLContext ctx, Record rs) {
                return new SQLAccountRansom(rs);
            }

            @Override
            protected void save(DSLContext ctx, Account.AccountRansom accountRansom) {
                ctx.mergeInto(ACCOUNT_RANSOM, ACCOUNT_RANSOM.ACCOUNT_ID, ACCOUNT_RANSOM.RANSOM, ACCOUNT_RANSOM.APPLY_TIME, ACCOUNT_RANSOM.HEIGHT,ACCOUNT_RANSOM.LATEST)
                        .key(ACCOUNT_ASSET.ACCOUNT_ID)
                        .values(accountRansom.getAccountId(), accountRansom.getRansom(), accountRansom.getApplyTime(), Burst.getBlockchain().getHeight(), true)
                        .execute();
            }
                ...
                for (Account account : accounts) {
                    ...
                    accountQueries.add(
                            ctx.mergeInto(ACCOUNT, ACCOUNT.ID, ACCOUNT.HEIGHT, ACCOUNT.CREATION_HEIGHT, ACCOUNT.PUBLIC_KEY, ACCOUNT.KEY_HEIGHT, ACCOUNT.BALANCE,
                                    ACCOUNT.UNCONFIRMED_BALANCE, ACCOUNT.FORGED_BALANCE, ACCOUNT.NAME, ACCOUNT.DESCRIPTION, ACCOUNT.LATEST, ACCOUNT.LEVEL, ACCOUNT.PLEDGE)
                                    ...
                                    .values(account.getId(), height, account.getCreationHeight(), account.getPublicKey(), account.getKeyHeight(),
                                            account.getBalanceNQT(), account.getUnconfirmedBalanceNQT(), account.getForgedBalanceNQT(), account.getName(), account.getDescription(), true, account.getLevel(), account.getPledge())
                    );
                }
    ...
    @Override
    public VersionedEntityTable<Account.AccountRansom> getAccountRansomTable() {
        return accountRansomTable;
    }

    @Override
    public DbKey.LinkKeyFactory<Account.AccountRansom> getAccountRansomKeyFactory() {
        return accountRansomDbKeyFactory;
    }
    ...
    static class SQLAccountRansom extends Account.AccountRansom {
        SQLAccountRansom(Record rs) {
            super(rs.get(ACCOUNT_RANSOM.ACCOUNT_ID),
                    rs.get(ACCOUNT_RANSOM.RANSOM),
                    rs.get(ACCOUNT_RANSOM.APPLY_TIME),
                    //rs.get(ACCOUNT_RANSOM.STATUS),
                    accountRansomDbKeyFactory.newKey(rs.get(ACCOUNT_RANSOM.ACCOUNT_ID), rs.get(ACCOUNT_RANSOM.APPLY_TIME))
            );
        }
    }
    ...
    class SqlAccount extends Account {
        SqlAccount(Record record) {
            ...
            this.level = record.get(ACCOUNT.LEVEL);
            //this.status = record.get(ACCOUNT.STATUS);
            this.pledge = record.get(ACCOUNT.PLEDGE);
            //this.applyTime = record.get(ACCOUNT.APPLY_TIME);
            //this.lastRedeemTime = record.get(ACCOUNT.LAST_REDEEM_TIME);
        }
    }
