...
public class AccountServiceImpl implements AccountService {
    ...
    private final LinkKeyFactory<Account.AccountRansom> accountRansomKeyFactory;
    ...
    public AccountServiceImpl(AccountStore accountStore, AssetTransferStore assetTransferStore) {
        ...
        this.accountRansomTable = accountStore.getAccountRansomTable();
        this.accountRansomKeyFactory = accountStore.getAccountRansomKeyFactory();
 ...
 /*   public Collection<Account.AccountRansom> getAllAccountRansoms(long accountId) {
        return accountRansomTable.getManyBy(ACCOUNT_RANSOM.ACCOUNT_ID.eq(accountId), 0, -1);
    }*/

    @Override
    public void insertAccountRansom(Account.AccountRansom accountRansom) {
        accountRansomTable.insert(accountRansom);
    }

    @Override
    public Account.AccountRansom insertNewAccountRansom(long accountId, long ransom, long applyTime) {
        //Account account = accountTable.get(accountBurstKeyFactory.newKey(id));
        Account.AccountRansom accountRansom;
        BurstKey burstKey = accountRansomKeyFactory.newKey(accountId, applyTime);
        accountRansom = new Account.AccountRansom(accountId, ransom, applyTime, burstKey);
        accountRansomTable.insert(accountRansom);
        return accountRansom;
    }

    @Override
    public Account.AccountRansom getAccountRansom(long accountId, long applyTime) {
        //Account account = accountTable.get(accountBurstKeyFactory.newKey(id));
        BurstKey burstKey = accountRansomKeyFactory.newKey(accountId, applyTime);
        Account.AccountRansom accountRansom = accountRansomTable.get(burstKey);
        return accountRansom;
    }

    @Override
    public void delete(Account.AccountRansom accountRansom) {
        accountRansomTable.delete(accountRansom);
    }
